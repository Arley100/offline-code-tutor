...
- **Web app foundation:** the EvalForge / TutorBench Local app now lives in `web/`
  ...
  software with demo placeholder auth.
...
manual scoring, Ticket 5 added a comparison dashboard, and **Ticket 6 added
Markdown report export** of that evidence. Imported metrics are shown as-is and
missing metrics are shown as unavailable, never zero. Manual scores are
human-assigned only; the web app does not auto-score with AI, does not mutate
imported artifact JSON, and does not claim statistical certainty.
